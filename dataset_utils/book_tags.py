...
def clean(dataset):
    ...
    discard_rows = dataset[dataset["goodreads_book_id"].isin(bookdata["discarded"])]
    print(f"Discarding {len(discard_rows)}: no such book.\nWould break FK constraints in MySQL")
    dataset = dataset[~dataset["goodreads_book_id"].isin(bookdata["discarded"])]
    print(f"Dropping duplicates\nWould break PK constraints in MySQL")
    dataset.drop_duplicates(subset = ["goodreads_book_id","tag_id"], inplace=True)
    print(f"Dropping any records which don't have a matching book")
    dataset = dataset[dataset["goodreads_book_id"].isin(bookdata["book_id"])] #  this references book_id not id 
    print(f"Saving the first 100k rows and dropping the rest.")
    dataset = dataset.head(100000)
    target_file = path_of(concat_path(path_of(__file__).parent.parent, path_of("dataset/book_tags.csv")))
